...
const getJobs = (req, res) => {
    ...
    let findParams = {};
    let sortParams = {dateOfPosting: -1};
...
const getRecommendedJobs = (req, res) => {
    ...
    Job.aggregate(arr)
        .then((posts) => {
            ...
            //fetch user profile and assign it to profile variable the find the skills of the user
            JobApplicant.findOne({userId: user._id}).then((profile) => {
                if (profile == null) {
                    res.status(404).json({
                ...
                let matchedJobs = matchJobsToProfile(jobs, profile);
...
function matchJobsToProfile(jobs, profile) {
    const matchedJobs = [];

    jobs.forEach(job => {
        const jobSkills = job.skillsets;
        const userSkills = profile.skills;

        const matchedSkills = userSkills.filter(skill => jobSkills.includes(skill));
        const score = matchedSkills.length / userSkills.length; // Calculate match score
    ...
    if (user.type == "recruiter") {
        Recruiter.findOne({userId: user._id})
            .then((recruiter) => {
                ...
                if (data.companyUrl) {
                    recruiter.companyUrl = data.companyUrl;
                }
                if (data.industrySector) {
                    recruiter.industrySector = data.industrySector;
                }
                if (data.companyLogo) {
                    recruiter.companyLogo = data.companyLogo;
                }
                recruiter
                    .save()
                    .then(() => {
        ...
        JobApplicant.findOne({userId: user._id})
            .then((jobApplicant) => {
                ...
                if (data.githublink) {
                    jobApplicant.githublink = data.githublink;
                }
                if (data.instagram) {
                    jobApplicant.instagram = data.instagram;
                }
                if (data.facebook) {
                    jobApplicant.facebook = data.facebook;
                }
                if (data.linkedin) {
                    jobApplicant.linkedin = data.linkedin;
                }
                if (data.resume) {
                    jobApplicant.resume = data.resume;
                }
                if (data.schoolrecommendation) {
                    jobApplicant.schoolrecommendation = data.schoolrecommendation;
                }
                if (data.lecturerrecommendation) {
                    jobApplicant.lecturerrecommendation = data.lecturerrecommendation;
                }
                if (data.insurancecover) {
                    jobApplicant.insurancecover = data.insurancecover;
                }
                if (data.profile) {
                    jobApplicant.profile = data.profile;
                }
                if (data.profilevideo) {
                    jobApplicant.profilevideo = data.profilevideo;
                }
                if (data.workexperienceYear) {
                    jobApplicant.experienceYear = data.data.workexperienceYear;
                }
...
const nodemailer = require('nodemailer'); // You need nodemailer to send emails
...
const forgotPassword = async (req, res) => {
    try {
        ...
        const token = jwt.sign({ email }, "Intech", { expiresIn: "1h" });
        ...
        const transporter = nodemailer.createTransport({
            ...
            auth: {
                user: "jenny.njogs@gmail.com", // replace with your email
                pass: "zsrapcurpjogwcsf",   // replace with your email password or app password
            },
        });

        // Email options
        const mailOptions = {
            from: '"INTECH jenny.njogs@gmail.com',
            to: email,
...
const isPremium=(req,res)=>{
    ...
    PaymentModel.find({userId}).then((payment)=>{
...
// const bcrypt = require('bcryptjs');    // To hash new passwords

const resetPassword = async (req, res) => {
    try {
        const { token } = req.params; // The token comes from the URL
        const { newPassword } = req.body; // The new password comes from form
        ...
        // Verify the token
        const decoded = jwt.verify(token, "Intech"); // Ideally use process.env.JWT_SECRET
        ...
        // const hashedPassword = jwt.sign(newPassword, "jwt_secret", { expiresIn: "1h" });
...
const savedJobs = async (req, res) => {
    ...
    if (user.type !== "applicant") {
        return res.status(400).json({
            message: "You are not allowed to access saved jobs",
        });
    }
    ...
    try {
        ...
        // Use Promise.all to wait for all Job queries to complete
        await Promise.all(savedJobs.map(async (savedJob) => {
            const job = await Job.findOne({ _id: savedJob.jobId });
            if (job) {
                jobs.push({
                    jobId: job._id,
                    title: job.title,
...
const getPayment = async (req, res) => {
    ...
    _phone = _phone.startsWith("0") ? `254${_phone.slice(1)}` : _phone;

    const MPESA_CONFIG = {
        'CONSUMER_KEY': 'WUXzACOooTl88ulz8KcehEHP7RvG2lV7',
        'CONSUMER_SECRET': 'zA5xvtAZKNiXDhJ6',
        'HOST_NAME': 'https://dbcd-128-203-207-30.ngrok-free.app/',
    ...
    try {
        const result = await stkPush(MPESA_CONFIG, {
            amount: 1,
            phone: _phone,
            accountReference: 'INTECH JOB PORTAL',
            transactionDesc: 'JOB PORTAL SUBSCRIPTION',
            shortCode: '4580690'
        });
  ...
  dateOfPayment
        ...
        res.json({message: "Payment successful", data: p});


    } catch (err) {
        console.error('Failed to push STK:', err);
    }
...
/**
 * Initiates a Lipa Na MPesa Online (STK Push)
 * @param {Object} config - MPESA_CONFIG
 * @param {Object} params - Payment details
 * @param {number} params.amount - Amount to be charged
 * @param {string} params.phone - Customer phone number (e.g., 2547XXXXXXX)
 * @param {string} params.accountReference - Reference (order/invoice ID)
 * @param {string} params.transactionDesc - Description
 * @returns {Object} MPesa STK push API response
 */
async function stkPush(config, {amount, phone, accountReference, transactionDesc}) {
    try {
        // Step 1: Get access token
        const auth = Buffer.from(`${config.CONSUMER_KEY}:${config.CONSUMER_SECRET}`).toString('base64');
        const tokenRes = await axios.get(
            `${config.SAFARICOM_API}/oauth/v1/generate?grant_type=client_credentials`,
            {
        ...
        const password = Buffer.from(`${config.SHORT_CODE}${config.PASS_KEY}${timestamp}`).toString('base64');

        // Step 3: Make STK Push request
        const res = await axios.post(
            `${config.SAFARICOM_API}/mpesa/stkpush/v1/processrequest`,
            {
                ...
                PhoneNumber: phone,
                CallBackURL: `${config.HOST_NAME}mpesa/callback/`, // make sure your callback path is correct
